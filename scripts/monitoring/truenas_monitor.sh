...
set -euo pipefail
# ------------------------------------------------------------------
# TrueNAS Container 48-Hour Stability Monitor - Final Fixed Version
# Version: 3.2 - Homelab GitOps Auditor Integration
...
get_container_metrics_final() {
    ...
    # Get status and config with proven curl timeout parameters
    local status_data=""
    local config_data=""
    ...
    # Write to temp files with unique names
    local temp_status="/tmp/status_${container_id}_$$.json"
    local temp_config="/tmp/config_${container_id}_$$.json" 
    local temp_meta="/tmp/meta_${container_id}_$$.json"
    
    echo "$status_data" > "$temp_status"
    echo "$config_data" > "$temp_config"
    
    # Write container metadata to eliminate shell variable interpolation completely
    cat > "$temp_meta" << EOF
{
...
EOF
    ...
    # Process with Python using only temp files - NO shell variable interpolation
    local metrics=$(python3 << 'EOF_PYTHON'
import json
import sys
from datetime import datetime

try:
    # Read all data from temp files
    with open('/tmp/status_' + str(CONTAINER_ID) + '_' + str(PID) + '.json', 'r') as f:
        status_response = json.load(f)
    with open('/tmp/config_' + str(CONTAINER_ID) + '_' + str(PID) + '.json', 'r') as f:
        config_response = json.load(f)
    with open('/tmp/meta_' + str(CONTAINER_ID) + '_' + str(PID) + '.json', 'r') as f:
        meta = json.load(f)
    ...
    # Create result using metadata from file
    result = {
    ...
    # Create error result using metadata if available
    try:
        with open('/tmp/meta_' + str(CONTAINER_ID) + '_' + str(PID) + '.json', 'r') as f:
            meta = json.load(f)
        error_result = {
...
EOF_PYTHON
    ...
    # Wait, that still has the variables. Let me use sed to replace them first
    local python_script="/tmp/process_${container_id}_$$.py"
    
    cat > "$python_script" << 'EOF_PYTHON'
import json
import sys
import os
from datetime import datetime
...
try:
    ...
    # Read all data from temp files
    with open(f'/tmp/status_{container_id}_{pid}.json', 'r') as f:
        status_response = json.load(f)
    with open(f'/tmp/config_{container_id}_{pid}.json', 'r') as f:
        config_response = json.load(f)
    with open(f'/tmp/meta_{container_id}_{pid}.json', 'r') as f:
        meta = json.load(f)
...
EOF_PYTHON
    ...
    # Run Python processing with environment variables
    local metrics=$(CONTAINER_ID="$container_id" PID="$$" python3 "$python_script" 2>/dev/null)
    
    # Cleanup temp files
    rm -f "$temp_status" "$temp_config" "$temp_meta" "$python_script" 2>/dev/null || true
...
collect_stability_sample() {
    ...
    for container_id in "${!TRUENAS_CONTAINERS[@]}"; do
        ...
        # Get metrics
        local container_data=$(get_container_metrics_final "$container_id" "$name" "$type" "$priority" "$original_id")
