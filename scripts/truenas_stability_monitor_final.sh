...
set -euo pipefail
# ------------------------------------------------------------------
# TrueNAS Container 48-Hour Stability Monitor - Final Version
# Version: 3.0 - Homelab GitOps Auditor Integration
...
get_container_metrics_curl() {
    ...
    # Get status and config with curl, handle errors gracefully
    local status_data=""
    local config_data=""
    
    # Get container status with proper timeout parameters based on web research
    ...
    # Process with Python - but avoid async
...
try:
    ...
    config_response = json.loads('$config_data')
    ...
    memory_used = safe_get(status_data, 'mem', 0)
    memory_max = safe_get(status_data, 'maxmem', 1)
    memory_percent = round((memory_used / memory_max) * 100, 2) if memory_max > 0 else 0
    cpu_percent = round(safe_get(status_data, 'cpu', 0) * 100, 2)
    uptime_hours = round(safe_get(status_data, 'uptime', 0) / 3600, 2)
    
    # Determine health status
    health = 'healthy'
    container_status = safe_get(status_data, 'status', 'unknown')
    ...
    rootfs = safe_get(config_data, 'rootfs', '')
...
collect_stability_sample() {
    ...
    for container_id in "${!TRUENAS_CONTAINERS[@]}"; do
        ...
        # Get metrics
        local container_data=$(get_container_metrics_curl "$container_id" "$name" "$type" "$priority" "$original_id")
        
        # Parse health status and CSV data with error handling
...
" 2>/dev/null || echo "error,error,error,error,error,error,error,error,error,error,error")
        fi
...
analyze_stability_results() {
    ...
    # Simple analysis using tail and basic metrics
